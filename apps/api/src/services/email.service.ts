...
export const emailService = {
  ...
  /**
   * Send Hiring Application Confirmation Receipt (Job / Internship)
   */
  async sendHiringApplicationReceipt(
    email: string,
    name: string,
    type: string,
    department: string,
    ...
    const htmlContent = `
      <h2 style="margin-top: 0; color: #111827; font-size: 20px;">Application Received: ${type} at TSC</h2>
      <p>Hello <strong>${name}</strong>,</p>
      <p>Thank you for applying to join <strong>THE STUDENT CHAPTERS™</strong>.</p>
      <div style="background: #F9FAFB; border-left: 4px solid #1457A2; padding: 16px 20px; margin: 20px 0; border-radius: 6px;">
        <p style="margin: 0; font-size: 15px; font-weight: 700; color: #111827;">Role / Domain: ${department}</p>
        <p style="margin: 6px 0 0; font-size: 12px; color: #6B7280; text-transform: uppercase; letter-spacing: 0.05em;">Type: ${type} &bull; Reference Code: #${refId}</p>
      </div>
      <p>Our recruitment &amp; editorial team reviews applications on a rolling basis. If your profile matches our active editorial or engineering needs, we will reach out with next steps.</p>
      <p>In the meantime, feel free to explore our platform, read student stories, and check our latest campus dispatches.</p>
      <div style="text-align: center; margin: 24px 0;">
        <a href="${env.clientUrl}/about" class="btn" style="color: #ffffff !important;">Explore Our Mission &rarr;</a>
      </div>
      <p style="font-size: 12px; color: #9CA3AF; border-top: 1px solid #E5E7EB; padding-top: 16px;">
        This automated receipt was generated by THE STUDENT CHAPTERS recruitment portal for reference #${refId}.
      </p>
    `;
