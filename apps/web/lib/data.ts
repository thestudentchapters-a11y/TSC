...
async function withApi<T>(path: string, fallback: T): Promise<T> {
  ...
  try {
    ...
    return (json.data ?? (json as unknown as T)) ?? fallback;
  } catch (err) {
...
/* News */
export const getArticles = () => withApi<Article[]>('/api/news?limit=24', demoArticles);
...
/* Stories */
export const getStories = () => withApi<Story[]>('/api/stories', demoStories);
...
/* Campuses */
export const getCampuses = () => withApi<Campus[]>('/api/campuses', demoCampuses);
...
/* Podcast */
export const getEpisodes = () => withApi<PodcastEpisode[]>('/api/podcasts', demoEpisodes);
...
/* Events */
export const getEvents = () => withApi<TscEvent[]>('/api/events', demoEvents);
...
/* Opportunities */
export const getOpportunities = () => withApi<Opportunity[]>('/api/opportunities', demoOpportunities);

/* Current Affairs */
export const getEditions = () => withApi<CurrentAffairsEdition[]>('/api/current-affairs', demoEditions);
...
/* Legal Awareness */
export const getLegalArticles = () => withApi<LegalArticle[]>('/api/legal-awareness', demoLegalArticles);
