    ...
    <article className="card-base card-hover group flex h-full flex-col gap-4 p-5">
      ...
      <div className="mt-auto flex items-center justify-between gap-3 border-t border-hairline pt-4">
        ...
        {opportunity.applicationUrl ? (
          <Button href={opportunity.applicationUrl} size="sm" arrow className="shrink-0">
            Apply
          </Button>
        ) : (
          <Button size="sm" variant="outline" arrow href={`/career/${opportunity.type === 'Job' ? 'jobs' : opportunity.type === 'Internship' ? 'internships' : opportunity.type === 'Fellowship' ? 'fellowships' : 'careers'}`} className="shrink-0">
            View &amp; Apply
          </Button>
        )}
      </div>
