...
export function OpportunityCard({ opportunity }: { opportunity: Opportunity }) {
  ...
  return (
    <article className="card-base card-hover group flex flex-col gap-4 p-5">
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-display text-[16px] font-bold leading-snug transition-colors group-hover:text-brand">
              {opportunity.title}
            </h3>
      ...
      <p className="line-clamp-2 text-sm leading-6 text-muted">{opportunity.description}</p>

      <div className="flex flex-wrap gap-1.5">
        {opportunity.skills.slice(0, 4).map((s) => (
          <span key={s} className="rounded-full border border-hairline bg-white px-2.5 py-1 text-[11px] font-medium text-ink/70">
            {s}
          </span>
        ))}
      </div>
      ...
      <div className="mt-auto flex items-center justify-between gap-3 border-t border-hairline pt-4">
        <p className="flex flex-col gap-0.5 text-[11px] leading-4 text-muted">
          <span className="inline-flex items-center gap-1">
            <GraduationCap aria-hidden className="h-3 w-3 text-brand" />
            {opportunity.eligibility}
          </span>
          <span className="inline-flex items-center gap-1">
            <CalendarDays aria-hidden className="h-3 w-3 text-gold-deep" />
            Deadline: {formatDate(opportunity.deadline)}
          </span>
        </p>
        {opportunity.applicationUrl ? (
          <Button href={opportunity.applicationUrl} size="sm" arrow>
            Apply
          </Button>
        ) : (
          <Button size="sm" variant="outline" arrow href={`/career/${opportunity.type === 'Job' ? 'jobs' : opportunity.type === 'Internship' ? 'internships' : opportunity.type === 'Fellowship' ? 'fellowships' : 'careers'}`}>
            View &amp; Apply
          </Button>
        )}
      </div>
